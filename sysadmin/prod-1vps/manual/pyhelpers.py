...
def create_droplets():
  ...
  for hostname in new_droplets:
    ...
    payload = {
      'name': hostname,
      'size': new_droplets[hostname]['size'],
      'image': new_droplets[hostname]['image'],
      'region': new_droplets[hostname]['region'],
      'ssh_keys': hostname_ssh_key_ids[hostname],
      'private_networking': new_droplets[hostname]['private_networking'],
      'backups': new_droplets[hostname]['backups']
    }
    print 'DO droplet creation payload:'
    print payload
    headers = {'Authorization': 'Bearer ' + os.environ['DO_TOKEN']}
    r = requests.post('https://api.digitalocean.com/v2/droplets', data=payload, headers=headers)
    print 'DO response: '
    print r.json()
    if r.json()['status'] != 'OK':
  ...
  with open(os.path.abspath(os.path.join(os.path.dirname(__file__), '../production')), 'w') as outfile:
    for category in new_droplets_config:
      ...
      for hostname in new_droplets_config[category]:
        outfile.write(hostname + '    ansible_ssh_host=' + droplets[hostname]['ip_address'] + '    ansible_ssh_private_key_file=~/.ssh/' + hostname + '\n')
      outfile.write('\n')

def update_json_keys_ips():
  """ Adds appropriate localhost IPs to the /dist/lib/config/keys/prod/ node.json and python.json files.
  ...
  node_json['db']['host'] = "127.0.0.1"
  python_json['db']['host'] = "127.0.0.1"
  node_json['redis']['host'] = "127.0.0.1"
  node_json['python']['host'] = "127.0.0.1"
