...
exports.makeSuggestion = function(req, res) {
  ...
  // currently that is just if the user has voted 100 or more times
  // on client-side we only request suggestions every 100 votes, but it will take some more sophisticated
  // implementation to ensure this on the server-side, because we want to allow the user to keep requesting
  // suggestions after every vote if there was an error in generating suggestions the last time they hit a 
  // multiple of 100 votes and they haven't successfully gotten suggestsions yet
  var voteCount = req.user.voteCount;
  if (voteCount < 100) {
    var requiredVotes = 100 - voteCount;
