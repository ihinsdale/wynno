  ...
  if (tweet.__entities.hasOwnProperty('media') && Array.isArray(tweet.__entities.media)) {
    ...
    for (var i = 0; i < tweet.__entities.media.length; i++) {
      ...
      if (mediaEntity.type === 'photo') {
        ...
        tweet.renderedText += "<div class='tweetPicLinkWrapperContainer'><a class='tweetPicLinkWrapper' href='"
                              + mediaEntity.url + "' target='_blank'><div class='tweetPicContainer'><img class='tweetPic' src='"
                              + mediaEntity.media_url_https + "'></div></a></div>";
      }
    }
  }
  // 2. YouTube clips
  if (tweet.__entities.hasOwnProperty('urls') && Array.isArray(tweet.__entities.urls)) {
    ...
    for (var j = 0; j < tweet.__entities.urls.length; j++) {
      ...
      if (parsedUrl.hostname === 'www.youtube.com') {
        ...
        // displayed after the text of the tweet
        tweet.renderedText += "<div class='videoContainer'><div class='flex-video widescreen'><iframe src='//www.youtube.com/embed/"
                              + parsedUrl.query.v + "' frameborder='0' scrolling='no' allowtransparency='true'></iframe></div></div>"
      } else if (parsedUrl.hostname === 'youtu.be') {
        tweet.renderedText += "<div class='videoContainer'><div class='flex-video widescreen'><iframe src='//www.youtube.com/embed/"
                              + parsedUrl.pathname.slice(1) + "' frameborder='0' scrolling='no' allowtransparency='true'></iframe></div></div>"
      }
    }
  }
};
